...
\item Explique las diferencias entre direcciones logicas y direcciones físicas.\\
  ...
  Aumentando así la rapidez con la cual se propaga ya que no se intersecta con ningún otro haz de luz que haya
  ingresado al material y este viaja de forma lineal.Este tipo de conexión de fibra optica es mas cara debido a que
  para propagar ese haz de luz es necesaria una fuente de luz más cara.
  
  \item Señale las principales caracterısticas del cable de ﬁbra optica multimodo. \\
  
  Posee multiples haces de luz dentro de su material dielectrico,aumentando la posibilidad de disminuir
  su rapidez en la que se propaga ya que al haber muchos haces de luz estos pueden chocar provocando asi
  problemas con su transmisión.Este tipo de conexión de fibra optica es mas barata debido a que su cavidad de el
  materia dielectrico es mas grande, y gracias a esto poder ocupar fuentes de luz más baratas
  
  \item Señale las principales caracterısticas del cable de ﬁbra optica multimodo con índice gradual(COMPLETAR)\\
  	
  	En este tipo de fibra óptica, el núcleo está constituido de varias capas concéntricas de material óptico 
  	con diferentes índices de refracción, causando que el rayo de luz de refracte poco a poco mientras viaja 
  	por el núcleo, pareciendo que el rayo se curva .

En estas fibras el número de rayos ópticos diferentes que viajan es menor que en el caso de la fibra multimodo índice escalonado 
...
-La fibra monomodo funciona tan solo con 1 haz de luz\\
-La fibra multimodo tiene mas de 1 haz de luz\\
...
\item  De la Figura 1 responda lo siguiente(102):
 a) Indique el número de dominios de colisiones y de dominios de broadcast. \\
b) Si se envía un frame desde el PC0 al PC1. Indique las direcciones IPs y MACs del frame que se transmite entre el PC0 y el Switch1. Indique las direcciones IPs y MACs del frame que se transmite entre el Switch1 y el PC1.\\ 
c) Si se envía un frame desde el PC0 al PC2. Indique las direcciones IPs y MACs del frame que se transmite entre el PC0 y el Switch1. Las direcciones IPs y MACs del frame que se transmite entre el Switch1 y el Switch0. Las direcciones IPs y MACs del frame entre el Switch0 y Switch2 y las direcciones IPs y MACs del frame entre el Switch2 y el PC2. \\
d) Dibuje la tabla ARP del PC1. 
e) Dibuje la tabla del Switch2.\\
a) –Dominio broadcast: 1
-Dominio de colisiones: 3
b)PC0 -> Switch1
Dst: -192.169.0.3
-0090.0CCA.0332
Src: -192.168.0.2
...
e) -192.168.0.6
-000D.BDB3.6989
\item De la Figura 2 responda lo siguiente(103): 
a) Indique el número de dominios de colisiones y de dominios de broadcast. 
b) Si se envía un frame desde el PC0 al PC1. Indique las direcciones IPs y MACs del frame que se transmite entre el PC0 y el Switch0. Indique las direcciones IPs y MACs del frame que se transmite entre el Switch0 y el PC1. 
c) Si se envía un frame desde el PC0 al PC2. Indique las direcciones IPs y MACs del frame que se transmite entre el PC0 y el Switch0. Las direcciones IPs y MACs del frame que se transmite entre el Switch0 y el Router0. Las direcciones IPs y MACs del frame entre el Router0 y Switch1 y las direcciones IPs y MACs del frame entre el Switch1 y el PC2. 
d) Dibuje la tabla ARP del PC1. 
e) Dibuje la tabla ARP del PC2. 
f) Dibuje la tabla del Switch1. 
g) Dibuje la tabla del Switch2.
a) Dominio de colisiones: 2
Dominio Broadcast: 2
b) b)PC0 -> Switch0
Dst: -192.169.0.3
-0090.0CCA.0332
