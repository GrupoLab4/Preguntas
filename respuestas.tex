...
\item ¿Por que es importante la etapa de preámbulo al comienzo de cada frame?.\\
    Por que le indica al recetorel comienzo de un frame, para que este se prepare para recibirlo.
...
FF?.\\
    ...
    El protocolo ARP es un estándar TCP/IP que asocia las direcciones IP y MAC de los equipos conectados a la red. Un ejemplo de ataque utilizando ARP es el ARP-Spoofing consiste en enviar paquetes ARP falsos para que un router llene su tabla ARP con información falsa, de esta manera todos los datos que se supone llegarían a nuestra víctima llegaran al pc con la dirección MAC que hayamos especificado en el paquete falso.\\
    ...
    Para dar espacio a nuevas direcciones MAC e IP de otros equipos que se estén usando más, ya que al estar en la tabla ARP el dispositivo no tendrá que mandar un paquete en busca de donde está el equipo de destino y el proceso de transferencia de datos será mas rápido.\\
...
Por que en las redes conmutados los equipos no comparten un canal unico, en cambio las redes tradicionales se comparte el canal de transmision \\
